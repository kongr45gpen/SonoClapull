...
int main() {
    ...
    for (int i = 0; i < fftSize / 2.0; i++) {
        ...
        for (double efactor : factors1) {
            if (fabs(efactor - factor) < tolerance) {
                ...
                frequencies.emplace(fabs(efactor - factor), average);
    ...
    for (pair<double, double> freq : frequencies) {
        // Calculate note
        double note = notes * log2(freq.second / tuning);
        ...
        cout << "Found factor " << fixed << setprecision(5) << setw(12)
             ...
             << " [" << setw(2) << hostileHarmonics.count(freq.second) << " hostile harmonics" << "]"
                << " ♪" << setw(5) << setprecision(2) << note - round(note) << "±" << setw(7) << setprecision(3) << setfill('0') << fabs(freq.second - noteFrequency) << setfill(' ');
        if (fabs(freq.second - noteFrequency) < hostileTolerance) {
